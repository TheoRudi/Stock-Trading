...
if setting_choice == "NASDAQ":
    ...
    tickers.append(str(setting_choice))
    singleTicker = True

if singleTicker:
    tickerBundles = tickers
else:
    tickerBundles = np.array_split(tickers, len(tickers)/5)
...
for bundle in tickerBundles:
    #Clearing financial_dirs to avoid memory overloads...
    financial_dir_cy = {}
    financial_dir_py = {}
    financial_dir_py2 = {}
    ...
    rankedStocks.append(f_score_df.sum().sort_values(ascending=False))

fileName = "F-Scores(s)_" + str(setting_choice) + "_" + str(time.time()) + ".txt"
text_file = open(fileName, "w")
text_file.write(str(rankedStocks))
text_file.close()

print("")
print("Top stocks ranked based on Piotroski F-Score:")
print(rankedStocks)
